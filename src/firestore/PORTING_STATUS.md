...
## Implemented
- Query completeness: `where_filter` with `and(...)` / `or(...)` composite filters (nested, encoded as `compositeFilter`), `delete_field()` sentinel for `update` and merge `set` (also honoured by the in-memory datastore), `FieldPath::canonical_string` backtick-quotes non-identifier segments like the JS SDK (a field literally named `a.b` is addressable via `FieldPath::new`), `from_dot_separated` rejects empty segments and `~*/[]`, snapshot cursors (`start_at_snapshot`, `start_after_snapshot`, `end_at_snapshot`, `end_before_snapshot`) including the `__name__` reference, implicit `orderBy` for inequality fields (`queryNormalizedOrderBy`), and a total cross-type value ordering in the local evaluator. Verified against the emulator by `tests/live_endpoints.rs`.
