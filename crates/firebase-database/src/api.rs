...
use crate::backend::{select_backend, CasOutcome, DatabaseBackend};
use crate::constants::DATABASE_COMPONENT_NAME;
use crate::error::{internal_error, invalid_argument, permission_denied, DatabaseError, DatabaseResult};
use crate::on_disconnect::OnDisconnect;
use crate::push_id::next_push_id;
use crate::query::{QueryBound, QueryIndex, QueryLimit, QueryParams};
use crate::realtime::{ListenSpec, Repo};
...
impl Database {
    ...
    pub fn connect_emulator(&self, host: &str, port: u16) -> DatabaseResult<()> {
        ...
        crate::backend::set_database_url_override(self.inner.app.name(), url);
...
impl DatabaseReference {
    ...
    ///
    /// `ServerValue` placeholders ([`server_timestamp`](crate::server_timestamp),
    /// [`increment`](crate::increment)) are resolved by the server, so an `increment` is
    ...
    /// # Examples
    /// ```rust,ignore
    /// # use firebase_database::{DatabaseReference, DatabaseResult};
    /// # use serde_json::json;
...
///
/// ```no_run
/// # use firebase_database::{connect_database_emulator, get_database};
...
pub async fn get_database(app: Option<FirebaseApp>) -> DatabaseResult<Arc<Database>> {
    ...
    let app = match app {
        ...
        None => {
            ...
            {
                firebase_core::app::get_app(None)
                    .await
...
mod tests {
    ...
    use firebase_core::app::{FirebaseApp, FirebaseAppConfig, FirebaseAppSettings, FirebaseOptions};
    ...
    async fn connect_emulator_points_every_channel_at_the_emulator() {
        ...
        assert_eq!(
            crate::backend::database_url_for(&app).as_deref(),
    ...
    async fn connect_emulator_is_refused_once_listeners_exist() {
        ...
        assert_eq!(err.code, crate::error::DatabaseErrorCode::InvalidArgument);
    }
}
