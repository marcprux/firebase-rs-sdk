...
use crate::error::{map_server_error, AuthError, AuthErrorCode, AuthResult};
use crate::model::MfaEnrollmentInfo;
use crate::model::{
...
use crate::persistence::indexed_db::IndexedDbPersistence;
...
impl Auth {
    ...
    /// ```no_run
    /// # use std::sync::Arc;
    /// # use firebase_auth::{Auth, FilePersistence};
    ...
    /// # Examples
    /// ```rust,no_run
    /// # use firebase_auth::doctest_support::get_mock_auth;
    /// # use firebase_auth::AuthError;
    ...
    /// ```rust,no_run
    /// # use std::sync::Arc;
    /// # use firebase_auth::doctest_support::{get_mock_auth, auth::MockVerifier};
    /// # use firebase_auth::AuthError;
    /// # async fn run() -> Result<(), AuthError> {
    /// # let auth = get_mock_auth(None).await;
    ...
    /// let sms_code = "123456";
    /// let credential = firebase_auth::PhoneAuthProvider::credential(verification_id, sms_code);
    ...
    /// # fn demo(auth: &Auth) {
    /// let unsubscribe = auth.on_auth_state_changed(|user: &Option<std::sync::Arc<firebase_auth::User>>| {
    ///     match user {
    ...
    pub fn token_provider(self: &Arc<Self>) -> TokenProviderArc {
        crate::token_provider::auth_token_provider_arc(self.clone())
    }
    ...
    /// # async fn run() -> Result<(), AuthError> {
    /// # let auth = get_mock_auth(None).await;
    /// let settings = firebase_auth::ActionCodeSettings {
    ...
    pub async fn reauthenticate_with_oauth_credential(&self, credential: AuthCredential) -> AuthResult<Arc<User>> {
        ...
        if result.user.uid() != user.uid() {
            return Err(AuthError::Server(crate::AuthServerError::new(
                "USER_MISMATCH",
...
/// [`FilePersistence`](crate::FilePersistence) (or your own
/// [`AuthPersistence`](crate::AuthPersistence)) pointing at a location only your application
...
/// # use firebase_auth::{initialize_auth, FilePersistence};
...
mod tests {
    use super::*;
    use crate::error::{AuthErrorCode, MultiFactorAuthErrorCode};
    use crate::types::{ActionCodeSettings, AndroidSettings, ApplicationVerifier, IosSettings};
    use crate::{
    ...
    use firebase_core::test_support::{start_mock_server, test_firebase_app_with_api_key};
    ...
    fn dead_sessions_are_told_apart_from_transient_failures() {
        let dead = |code: &str| AuthError::Server(crate::error::AuthServerError::new(code, None, Some(400)));
