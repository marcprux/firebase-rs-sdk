...
use crate::error::{internal_error, invalid_argument, FunctionsResult};
...
impl Functions {
    ...
    /// # Examples
    /// ```ignore
    /// # use firebase_functions::{get_functions, register_functions_component};
    /// # use firebase_functions::error::FunctionsResult;
    /// # use firebase_core::app::initialize_app;
    /// # use firebase_core::app::{FirebaseAppSettings, FirebaseOptions};
    /// # async fn demo() -> firebase_functions::error::FunctionsResult<()> {
...
pub struct CallableStream<Response> {
    body: crate::transport::StreamingBody,
    buffer: String,
    finished: bool,
...
{
    fn new(body: crate::transport::StreamingBody) -> Self {
        Self {
            body,
            buffer: String::new(),
            finished: false,
    ...
    fn process_line(&mut self, line: &str) -> FunctionsResult<StreamEvent> {
        ...
        if object.contains_key("error") {
            let error = crate::error::error_for_http_response(0, Some(&json))
    ...
    Response: serde::de::DeserializeOwned,
{
    ...
    ///
    /// ```no_run
    /// # use firebase_functions::Functions;
    /// # async fn demo(functions: Functions) -> firebase_functions::error::FunctionsResult<()> {
    ...
    pub async fn stream_async(&self, data: &Request) -> FunctionsResult<CallableStream<Response>> {
        ...
        let body = crate::transport::open_callable_stream(request).await?;
...
mod tests {
    use super::*;
    use crate::error::FunctionsErrorCode;
    use firebase_core::app::initialize_app;
    use firebase_core::app::{FirebaseAppSettings, FirebaseOptions};
