...
use crate::storage::IndexedDbRemoteConfigStorage;
use crate::storage::{FetchStatus, InMemoryRemoteConfigStorage, RemoteConfigStorage, RemoteConfigStorageCache};
use crate::value::{RemoteConfigValue, RemoteConfigValueSource};
...
impl RemoteConfig {
    ...
    ///
    /// ```
    /// use firebase_remote_config::RemoteConfigSettingsUpdate;
    /// # use firebase_remote_config::get_remote_config;
    /// # use firebase_core::app::initialize_app;
    /// # use firebase_core::app::{FirebaseOptions, FirebaseAppSettings};
...
fn default_fetch_client(app: &FirebaseApp) -> Arc<dyn RemoteConfigFetchClient> {
    match build_fetch_client(app) {
        Ok(client) => client,
        Err(err) => {
            firebase_core::app::LOGGER.warn(format!("remote-config: falling back to noop fetch client: {}", err));
...
fn build_fetch_client(app: &FirebaseApp) -> RemoteConfigResult<Arc<dyn RemoteConfigFetchClient>> {
    ...
    let sdk_version = format!("w:{}", firebase_core::app::SDK_VERSION);
    let base_url =
...
mod tests {
    use super::*;
    use crate::constants::{RC_CUSTOM_SIGNAL_KEY_MAX_LENGTH, RC_CUSTOM_SIGNAL_VALUE_MAX_LENGTH};
    use crate::error::{internal_error, RemoteConfigErrorCode};
    use crate::fetch::{FetchRequest, FetchResponse, RemoteConfigFetchClient};
    use crate::settings::{
        RemoteConfigSettingsUpdate, DEFAULT_FETCH_TIMEOUT_MILLIS, DEFAULT_MINIMUM_FETCH_INTERVAL_MILLIS,
    };
    #[cfg(not(target_arch = "wasm32"))]
    use crate::storage::FileRemoteConfigStorage;
    use crate::storage::{CustomSignals, FetchStatus};
    use firebase_core::app::initialize_app;
    use firebase_core::app::{FirebaseApp, FirebaseAppSettings, FirebaseOptions};
    ...
    fn set_config_settings_rejects_zero_timeout() {
        ...
        assert_eq!(
            ...
            crate::error::RemoteConfigErrorCode::InvalidArgument.as_str()
        );
    }
