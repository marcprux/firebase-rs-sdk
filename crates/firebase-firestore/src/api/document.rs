...
use crate::api::aggregate::{AggregateField, AggregateQuerySnapshot, AggregateSpec};
use crate::api::operations::{self, SetOptions};
use crate::api::query::{
    compute_doc_changes, ConvertedQuery, LimitType, Query, QuerySnapshot, QuerySnapshotMetadata, TypedQuerySnapshot,
};
use crate::api::snapshot::{DocumentSnapshot, TypedDocumentSnapshot};
use crate::error::{internal_error, invalid_argument, FirestoreResult};
use std::sync::Arc;

use crate::api::credentials;
#[cfg(not(target_arch = "wasm32"))]
use crate::api::listener::{listen_to_document, listen_to_query, ListenerRegistration};
use crate::remote::datastore::{Datastore, HttpDatastore, InMemoryDatastore, TokenProviderArc};
#[cfg(not(target_arch = "wasm32"))]
use crate::remote::listen::ListenTransport;
...
impl FirestoreClient {
    ...
    ///
    /// ```no_run
    /// # use firebase_firestore::{FirestoreClient, Query};
    ...
    pub fn on_document_snapshot<F>(
        &self,
        reference: &crate::api::reference::DocumentReference,
        callback: F,
    ...
    {
        let transport = self.listen_transport()?;
        let key = crate::model::DocumentKey::from_path(reference.path().clone())?;
    ...
    /// # use firebase_firestore::FirestoreClient;
    ...
    /// # }
    /// ```
    pub async fn for_app(app: firebase_core::app::FirebaseApp) -> FirestoreResult<Self> {
        let firestore = Firestore::from_arc(crate::get_firestore(Some(app)).await?);
    ...
    /// ```rust,no_run
    /// # use std::sync::Arc;
    /// # use firebase_firestore::doctest_support::get_mock_client;
    /// # use firebase_firestore::FirestoreResult;
    /// # async fn run() -> FirestoreResult<()> {
    /// # let client = get_mock_client(None).await;
    /// use std::collections::BTreeMap;
    ///
    /// use firebase_firestore::FirestoreValue;
    ///
    /// client
    ///     .update_doc(
    ///         "cities/sf",
    ...
    pub async fn set_doc_as<T: serde::Serialize + ?Sized>(
        ...
        let data = crate::value::to_document(value)?;
...
mod tests {
    use super::*;
    use crate::api::aggregate::{AggregateField, AggregateSpec};
    ...
    impl super::FirestoreDataConverter for PersonConverter {
        ...
        fn from_map(&self, value: &MapValue) -> FirestoreResult<Self::Model> {
            let first = match value.fields().get("first").and_then(|v| match v.kind() {
                ...
                Some(name) => name,
                None => return Err(crate::error::invalid_argument("missing first field")),
            };
            let last = match value.fields().get("last").and_then(|v| match v.kind() {
                ...
                Some(name) => name,
                None => return Err(crate::error::invalid_argument("missing last field")),
            };
