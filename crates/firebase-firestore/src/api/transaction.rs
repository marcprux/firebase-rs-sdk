...
use crate::api::converter::FirestoreDataConverter;
use crate::api::database::Firestore;
use crate::api::operations::{self, SetOptions};
use crate::api::reference::{ConvertedDocumentReference, DocumentReference};
...
use crate::remote::datastore::{ConditionalWrite, Datastore, Precondition, WriteOperation};
use crate::value::FirestoreValue;
use firebase_core::platform::runtime::sleep as runtime_sleep;

/// Tuning knobs for [`FirestoreClient::run_transaction`](crate::FirestoreClient::run_transaction).
#[derive(Clone, Debug)]
...
/// [`FirestoreClient::run_transaction`](crate::FirestoreClient::run_transaction).
...
mod tests {
    ...
    use crate::api::document::FirestoreClient;
    use crate::error::{not_found, unavailable};
    use crate::model::FieldPath;
    use crate::remote::datastore::{CommitResult, InMemoryDatastore};
    use crate::value::{MapValue, ValueKind};
    use crate::{AggregateDefinition, FieldTransform, QueryDefinition};
    ...
    fn retryable_error_classification_matches_js() {
        ...
        assert!(is_retryable_transaction_error(&crate::error::already_exists("x")));
        assert!(is_retryable_transaction_error(&unavailable("x")));
        assert!(is_retryable_transaction_error(&crate::error::internal_error("x")));
        assert!(!is_retryable_transaction_error(&invalid_argument("x")));
        assert!(!is_retryable_transaction_error(&not_found("x")));
        assert!(!is_retryable_transaction_error(&crate::error::permission_denied("x")));
    }
