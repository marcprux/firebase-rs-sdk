...
use crate::error::{internal_error, invalid_argument, unavailable, FirestoreError, FirestoreResult};
...
pub(crate) fn map_status_code(code: i32, message: &str) -> FirestoreError {
    ...
    match code {
        ...
        4 => crate::error::deadline_exceeded(message),
        5 => crate::error::not_found(message),
        6 => crate::error::already_exists(message),
        7 => crate::error::permission_denied(message),
        8 => crate::error::resource_exhausted(message),
        // A query that needs a composite index arrives as FAILED_PRECONDITION.
        9 => crate::error::failed_precondition(message),
        10 => crate::error::aborted(message),
        14 => unavailable(message),
        16 => crate::error::unauthenticated(message),
    ...
    fn target_changes_decode_with_their_resume_token() {
        ...
        match change {
            WatchChange::TargetChange(target_change) => {
                ...
                assert_eq!(target_change.read_time, Some(crate::model::Timestamp::new(5, 6)));
    ...
    fn a_rejected_target_keeps_the_servers_status_code() {
        let response = fs::ListenResponse {
            response_type: Some(fs::listen_response::ResponseType::TargetChange(fs::TargetChange {
                ...
                target_ids: vec![LISTEN_TARGET_ID],
                cause: Some(crate::remote::proto::google::rpc::Status {
                    code: 7,
