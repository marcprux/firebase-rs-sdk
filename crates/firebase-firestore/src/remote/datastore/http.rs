...
use crate::error::{internal_error, invalid_argument, FirestoreError, FirestoreErrorCode, FirestoreResult};
...
use crate::remote::structured_query::{encode_aggregation_body, encode_structured_query};
use crate::value::{FirestoreValue, MapValue};
use crate::AggregateDefinition;
use crate::FieldTransform;
...
use super::{
    credential_error, CommitResult, ConditionalWrite, Datastore, NoopTokenProvider, TokenProviderArc, WriteOperation,
    WriteResultInfo,
};
...
impl HttpDatastore {
    ...
    async fn build_request_context(&self) -> FirestoreResult<RequestContext> {
        let auth_token = self.auth_provider.get_token().await.map_err(credential_error)?;
        let app_check_token = self.app_check_provider.get_token().await.map_err(credential_error)?;
        let heartbeat_header = self
            .app_check_provider
            .heartbeat_header()
            .await
