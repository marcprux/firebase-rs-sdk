...
// Credentials are a cross-product concern, so the trait lives in the platform layer and Auth and
// App Check implement it there; Firestore only consumes it.
pub use firebase_core::platform::token::{NoopTokenProvider, TokenProvider, TokenProviderArc};

/// Turns a credential failure into a Firestore error at the boundary where tokens are consumed.
pub(crate) fn credential_error(error: firebase_core::platform::token::TokenError) -> crate::error::FirestoreError {
    crate::error::unauthenticated(error.to_string())
}
